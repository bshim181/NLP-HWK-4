...
class EarleyChart:
    ...
    def printItem(self, item):
        ...
        self.printItem(item[0].backpointers[0])
        if self.visited[item[0]]  == 1 and len(item[0].backpointers) == 1:
            self.result = self.result + ")"
            return;
        self.printItem(item[0].backpointers[1])
        self.result = self.result + ")"
    ...
    def _attach(self, item: Item, position:int) -> None:
        ...
        for customer in self.cols[mid].all():  # could you eliminate this inefficient linear search?
            if customer[0].next_symbol() == item[0].rule.lhs:
                ...
                if(index > 0):
                    if min(self.cols[position].getWeights(index), item[1] + customer[1]) == (item[1] + customer[1]):
                        self.cols[position].reprocess(new_item, item[1] + customer[1]);
...
class Agenda:
    ...
    def __init__(self) -> None:
        self._items: List[(Item, float)] = []  # list of all items that were *ever* pushed
    ...
    def reprocess(self, item, weight):
        #Have to change item's backpointers?
        self._items[self.checkExist(item)] = ((item, weight))
    ...
    with open(args.sentences) as f:
        for sentence in f.readlines():
            ...
            if sentence != "":  # skip blank lines
                ...
                # print the result
                maxWeight = chart.returnMaxProbability()
                #print(
                    #"Sentence Max Probability:" + str(maxWeight)
                #);
