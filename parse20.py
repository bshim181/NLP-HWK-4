...
log = logging.getLogger(Path(_file_).stem)  # For usage, see findsim.py in earlier assignment. 
...
def parse_args() -> argparse.Namespace: 
 ...
     def _init_(self, tokens: List[str], grammar: Grammar, progress: bool = False) -> None: 
 ...
             new_item = Item(rule, dot_position=0, start_position=position, weight=rule.weight, backpointers=()) 
 ...
         # if item is alrdy in the column but item has better weight than 
         # the one in the column: 
         # delete the old one completely, add item to end of self._items 
         # to process later 
 ...
             # self.erase(idx)  # completely remove fat item from push history of agenda 
             # self._items.append(item) 
             # self._index[item_info] = (len(self._items) - 1, item.weight) 
 ...
         """Given index of item to delete from the agenda, completely remove that item 
         so that it doesn't show up as a previously pushed item""" 
         self._items.pop(idx) 
  
     def _repr_(self): 
         """Provide a REPResentation of the instance for printing.""" 
         next = self._next 
         return f"{self._class.name_}({self._items[:next]}; {self._items[next:]})" 
 ...
     def _init_(self, start_symbol: str, *files: Path) -> None: 
 ...
         # return f"({self.start_position}, {self.weight}, {dotted_rule}, {self.backpointers})"  # matches notation on slides 
