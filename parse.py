...
class EarleyChart:
    ...
    def returnMaxProbability(self) -> float:
        ...
        for item in self.cols[-1].all():  # the last column
            if (item[0].rule.lhs == self.grammar.start_symbol  # a ROOT item in this column
                ...
                if min(self.maxWeights, item[1]) == item[1]:
                    self.maxItem = item
                    self.maxWeights = item[1]

        self.printItem(self.maxItem)
    ...
    def printItem(self, item):
        ...
        for i in range(len(item[0].backpointers)):
            if isinstance(item[0], Item) and not (item[0].rule,item[0].start_position, item[0].dot_position) in self.visited:
                self.visited[(item[0].rule, item[0].start_position, item[0].dot_position)] = 1
                self.result = self.result + "(" + item[0].rule.lhs;
            self.printItem(item[0].backpointers[i])
            if self.visited[(item[0].rule, item[0].start_position, item[0].dot_position)] == 1 and len(item[0].backpointers) == 1:
    ...
    def _predict(self, nonterminal: str, position: int) -> None:
        ...
        for rule in self.grammar.expansions(nonterminal):
            new_item = Item(rule, dot_position=0, start_position=position, backpointers=())
            self.cols[position].push(new_item, rule.weight)
    ...
    def _scan(self, item: Item, position: int) -> None:
        ...
        if position < len(self.tokens) and self.tokens[position] == item[0].next_symbol():
            new_item = item[0].with_dot_advanced(item[0].next_symbol())
            self.cols[position + 1].push(new_item, item[1])
            #trying reprocessing
    ...
    def _attach(self, item: Item, position:int) -> None:
        ...
        for customer in self.cols[mid].all():  # could you eliminate this inefficient linear search?
            if customer[0].next_symbol() == item[0].rule.lhs:
                new_item = customer[0].with_dot_advanced(item)
                self.cols[position].push(new_item, item[1] + customer[1]);
...
class Agenda:
    ...
    def __init__(self) -> None:
        self.items: List[Item, float] = []  # list of all items that were *ever* pushed
    ...
    def __len__(self) -> int:
        ...
        return len(self.items) - self._next
    ...
    def checkExist(self, item):
        if (item.rule, item.start_position, item.dot_position) in self._index:
            return self._index.get((item.rule, item.start_position, item.dot_position));
        else:
            return 0;

    def push(self, item, weight) -> None:
        """Add (enqueue) the item, unless it was previously added."""
        if (item.rule, item.start_position, item.dot_position) not in self._index:  # O(1) lookup in hash table
            self.items.append((item, weight))
            self._index[(item.rule, item.start_position, item.dot_position)] = len(self.items) - 1

        elif weight < self.getItems(self.checkExist(item))[1]:
            indexItem = self.checkExist(self.getItems(self.checkExist(item))[0]);
            # have to replace backpointers and weights
            self.reprocess(item, weight, indexItem)
            self.list.append(indexItem)




    def reprocess(self,  item, weight, indexItem):
        self.items[indexItem][0].backpointers = item.backpointers
        self.items[indexItem] = ((item, weight));


    def pop(self) -> (Item, float):
        """Returns one of the items that was waiting to be popped (dequeued).
        Raises IndexError if there are no items waiting."""
        if len(self) == 0:
            raise IndexError
        elif len(self.list) != 0:
            return self.items[self.list.pop(0)]
        item = self.items[self._next]
        self._next += 1
        return item;

    def all(self) -> Iterable[(Item, float)]:
    ...
    def __repr__(self):
        ...
        next = self._next
        return f"{self.__class__.__name__}({self.items[:next]}; {self.items[next:]})"
...
class Item:
    ...
    def with_dot_advanced(self, item) -> Item:
        # update item with new weights?
        if self.next_symbol() is None:
            raise IndexError("Can't advance the dot past the end of the rule")
        return Item(rule=self.rule, dot_position=self.dot_position + 1, start_position=self.start_position, backpointers= self.backpointers + (item,))
    ...
    with open(args.sentences) as f:
        for sentence in f.readlines():
            ...
            if sentence != "":  # skip blank lines
                ...
                print(chart.returnMaxProbability())
