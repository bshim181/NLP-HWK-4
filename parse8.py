...
class EarleyChart:
    ...
    def _predict(self, nonterminal: str, position: int) -> None:
        ...
        for rule in self.grammar.expansions(nonterminal):
            if (rule.rhs, position) not in self.predConst:
                self.predConst[(rule.rhs, position)] = position;
                new_item = Item(rule=rule, weights= rule.weight, backpointer=tuple(), dot_position=0, start_position=position)
    ...
    def _attach(self, item: Item, position: int) -> None:
        ...
        for customer in self.cols[mid].all():  # could you eliminate this inefficient linear search?
            ...
            if customer.next_symbol() == item.rule.lhs:
                new_item = customer.with_dot_advanced(weights=(item.weights + customer.weights), backpointer=tuple([item]));
                self.cols[position].push(new_item)

                """
                     if new_item not in self.completedConst:
                    self.completedConst[new_item] = item.weights + customer.weights;
                    self.cols[position].push(new_item)
                else:
                    if min(self.completedConst[new_item], item.weights + customer.weights) == (item.weights + customer.weights):
                        self.completedConst[new_item] = item.weights + customer.weights;
                        self.cols[position].push(new_item);
...
class Grammar:
    ...
    #Grammar Pre-Processing for every sentence.
...
class Item:
    ...
    def __hash__(self):
        return hash((self.rule, self.backpointer))

    def __eq__(self, item):
        return (item.rule == self.rule) and (item.backpointer == self.backpointer)
